...
def test_waiting_repetition_is_reserved(tmp_path):
    ...
    assert is_valid_repetition(
        make_trial(optuna.trial.TrialState.WAITING), result_path
    )
...
def test_startup_timeout_is_retried(tmp_path):
    ...
    assert not is_valid_repetition(
        make_trial(optuna.trial.TrialState.FAIL), result_path
    )
...
def test_mission_timeout_is_retried(tmp_path):
    ...
    assert not is_valid_repetition(
        make_trial(optuna.trial.TrialState.COMPLETE), result_path
    )
